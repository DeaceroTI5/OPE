USE Operacion
GO
-- EXEC SP_HELPTEXT 'OpeSch.OpeBackDigitalizacionDocumentos'
GO
ALTER PROCEDURE OpeSch.OpeBackDigitalizacionDocumentos
	@pnClaUbicacion		INT,
	@pnFechaIni			DATETIME = null /*= NULL*/,
	@pnFechaFin			DATETIME  = null/*= NULL*/,
	@pnEsRegeneracion	TINYINT = 0,
	@psFacturaFiltro	VARCHAR(8000)= '',
	@pnDebug			TINYINT = 0
AS
BEGIN
	...
	CREATE TABLE #TmpReporteFactura (
		  Id				INT IDENTITY(1,1)
		, IdFactura			INT
		, IdCertificado		INT
		, Reporte			VARBINARY(MAX)
	)
	...
	IF ISNULL(@pnEsRegeneracion,0) = 0
	BEGIN
	...
	BEGIN
		...
		--SELECT @pnFechaIni AS '@pnFechaIni', @pnFechaFin AS '@pnFechaFin'

		INSERT INTO #TempNoFactDigs (
			Seleccionar,  
			id, 
			IdFactura,  
			IdFacturaAlfanumerico, 
		...
		AND		((@pnFechaIni IS NULL AND @pnFechaFin IS NULL)
				OR (@pnFechaIni IS NOT NULL AND @pnFechaFin IS NOT NULL 
					AND (a.FechaEntSal >= @pnFechaIni AND a.FechaEntSal <= @pnFechaFin))
				)


		--SELECT	@pnIdReporteFactura = ISNULL(MAX(IdReporteFactura),0) + 1
		--FROM	OpeSch.OpeTmpReporteFactura WITH(NOLOCK)
	END
	...
	BEGIN
		...
		IF ISNULL(@pnEsRegeneracion,0) = 0
		BEGIN
			exec [OpeSch].[OPE_CU71_Pag4_Grid_GridDetalleFact_IU] @pnClaUbicacion,0,@IdTemp,2,1,@nIdFactura , @psIdFacturaAlfa , @psIdViaje
		END
		ELSE
		BEGIN
	...
	BEGIN
		...
		*/
		...
		END
		ELSE 
		IF ISNULL(@pnDebug,0) = 2
		BEGIN

			INSERT INTO #TmpReporteFactura (IdFactura, IdCertificado, Reporte)
			SELECT	  IdFactura		= @pnIdFactura

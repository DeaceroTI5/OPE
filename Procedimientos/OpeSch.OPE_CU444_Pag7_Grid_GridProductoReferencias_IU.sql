USE Operacion
GO
CREATE PROCEDURE OpeSch.OPE_CU444_Pag7_Grid_GridProductoReferencias_IU
   @pnClaUbicacion  INT  
    , @pnClaPlanCarga  INT  
    , @pnClaPedido   INT  
    , @pnClaPedidoDet  INT  
...
BEGIN  
 ...
 DECLARE @nClaAlmacen INT, @nClaSubAlmacen INT, @nClaSeccion INT, @nClaTipoInventario INT  
...
-- SELECT TOP 10 *   
 FROM OpeSch.OpeTraExistencias Ex  
 ...
 AND Ex.Cantidad > 0   
 IF EXISTS( SELECT 1 FROM OpeSch.OpeTraPlanCargaLocInv   
     WHERE ClaUbicacion = @pnClaUbicacion   
     AND IdPlanCarga = @pnClaPlanCarga  
 ...
 BEGIN  
  ...
  FROM OpeSch.OpeTraPlanCargaLocInv   
  WHERE ClaUbicacion = @pnClaUbicacion   
  AND IdPlanCarga = @pnClaPlanCarga  
  AND IdFabricacion = @pnClaPedido  
  AND IdFabricacionDet = @pnClaPedidoDet  
 ...
 FROM OpeSch.OpeRelArticuloTara AT  
 ...
 BEGIN   
  ...
  IF EXISTS   
   (  
    SELECT 1   
    FROM OpeSch.OpeTraPlanCargaTara   
    WHERE ClaUbicacion = @pnClaUbicacion   
    AND IdPlanCarga = @pnClaPlanCarga   
    AND IdFabricacion = @pnClaPedido   
    AND IdFabricacionDet = @pnClaPedidoDet  
  ...
  BEGIN  
   ...
   FROM OpeSch.OpeTraPlanCargaTara PCT  
   WHERE ClaUbicacion = @pnClaUbicacion   
   AND IdPlanCarga = @pnClaPlanCarga   
   AND IdFabricacion = @pnClaPedido   
   AND IdFabricacionDet = @pnClaPedidoDet  
 ...
 IF (ISNULL(@pnClaUbicacion, -1) > 0 AND ISNULL(@pnClaPlanCarga, -1) > 0)  
  IF NOT EXISTS (SELECT 1 FROM OpeSch.OpeLogPlanCarga WHERE ClaUbicacion = @pnClaUbicacion AND IdPlanCarga = @pnClaPlanCarga AND TipoRegistro = 30)  
   IF EXISTS (SELECT 1 FROM OpeSch.OpeTraPlanCargaLocInv WHERE ClaUbicacion = @pnClaUbicacion AND IdPlanCarga = @pnClaPlanCarga)  

...
BEGIN 
	...
	DECLARE @nIdViaje INT, @nIdBoleta INT, @sPlaca VARCHAR(12), @nIdTabular INT, @nIdTipoConcepto INT, @nClaCliente INT, 
		...
		@nEsStayTuff INT, @psIdioma VARCHAR(10), @pnEsValidarDigitalizacion INT, @PlantaManejaEstimacion INT, @TieneEstimacion INT
	...
	IF @pnClaUbicacion != 65 AND ISNULL(@pnIdOrdenEnvio, 0) = 0
	BEGIN          
   ...
   --END  
   CASE WHEN @pnClaUbicacion = 65 THEN 'en-Us'   
 ...
 AND  CertCli.BajaLogica = 0 
	...
	BEGIN
		...
		BEGIN
			...
			--REVISAMOS SI ES TRASPASO Y TIENE FABRICACIONES DE ESTIMACION
			IF EXISTS(SELECT 1 
			...
			BEGIN
				...
				INSERT INTO #tmpFabricacionFactura
							...
							idFactura,
							idViaje)
				SELECT DISTINCT A.ClaFormatoImpresion,
					D.IdFabricacionVenta,
					(CASE WHEN C.ClaPais = 1 THEN 1 ELSE 0 END),
					A.NoCopias, F.idFactura, F.idViaje
				FROM #tmpCfgMotivoFormatoImp A
				INNER JOIN OpeSch.OpeTraMovEntSal B WITH(NOLOCK) ON --'OpeSch.OpeTraMovEntSal'
					B.ClaUbicacion = @pnClaUbicacion AND 
					B.IdViaje = @nIdViaje
				INNER JOIN OpeSch.OpeTraViaje C WITH(NOLOCK) ON --'OpeSch.OpeTraViaje'
					C.ClaUbicacion = B.ClaUbicacion AND 
					C.IdViaje = B.IdViaje
   			    INNER JOIN OpeSch.OpeTraFabricacionEspejoEstimacion D WITH(NOLOCK)
					  ON	D.claUbicacionEstimacion = B.ClaUbicacion
					  AND	D.idFabricacionEstimacion = B.idFabricacion
				INNER JOIN OpeSch.OpeTraPlanCargaRemisionEstimacion E WITH(NOLOCK)
				ON e.ClaUbicacionEstimacion = C.ClaUbicacion
				AND e.idBoletaEstimacion = C.IdBoleta
				AND	e.idViajeEstimacion = C.IdViaje
				AND e.idPlanCargaEstimacion = C.IdPlanCarga
				INNER JOIN OpeSch.OpeTraMovEntSal F WITH(NOLOCK) ON --FALTA PASAR A TABLA TEMPORAL Y AGREGAR LINKEDSERVER PARA MULTISERVIDOR
					F.ClaUbicacion = E.ClaUbicacionVenta AND 
					F.IdViaje = E.idViajeVenta AND
					F.idFabricacion = D.IdFabricacionVenta
				WHERE ClaFormatoImpresion IN (18,19,20,21,22,23)
				AND B.IdFactura IS NULL	 -- ES TRASPASO
				AND F.idFactura IS NOT NULL -- ES ESTIMACION		
								
				IF EXISTS (SELECT 1 FROM #tmpFabricacionFactura)
					SET @TieneEstimacion = 1
				
				IF @psNombrePcMod = 'SOPORTE1'
				BEGIN
					SELECT * FROM #tmpCfgMotivoFormatoImp
					select * from #tmpFabricacionFactura
				END
					
				DECLARE @NoFabricacionesCalculadas INT, @NoFabricacionesEstimacion INT
				SELECT @NoFabricacionesCalculadas = COUNT(1)	
				FROM #tmpFabricacionFactura
				
				SELECT @NoFabricacionesEstimacion = COUNT(1)
				FROM opeSch.OpeTraMovEntSal B WITH(NOLOCK) 
				WHERE B.ClaUbicacion = @pnClaUbicacion AND 
						B.IdViaje = @nIdViaje
				
				
				--IF (ISNULL(@NoFabricacionesCalculadas,0) != ISNULL(@NoFabricacionesEstimacion,0) OR ISNULL(@TieneEstimacion,0) != 1)
				--BEGIN
					
				--	--exec msdb.dbo.sp_send_dbmail
				--	--@profile_name		= 'REIEmbarques',
				--	--@recipients			= 'lperaza@deacero.com',
				--	--@copy_recipients	= 'josmor@deacero.com', 
				--	--@importance			= 'Normal',
				--	--@subject			= 'Error en Viaje de estimacion:',-- + CONVERT(VARCHAR,@nIdViaje),
				--	--@body				= 'El viaje de Estimacion no calculó correctamente la impresion de facturas',
				--	--@body_format		= 'HTML',
		...
		BEGIN
			...
			SELECT ClaFormatoImpresion,
				NombreReporte = (CASE WHEN EsNacional = 1 THEN 'OPE_CU71_Pag1_Rpt_RemisionNacional' ELSE 'OPE_CU71_Pag1_Rpt_RemisionExportacion' END),
				CASE WHEN ISNULL(@TieneEstimacion,0) = 1 THEN 365 ELSE @pnClaUbicacion END,
				CASE WHEN ISNULL(@TieneEstimacion,0) = 1 THEN 365 ELSE @pnClaUbicacion END,
				ClaFactura = NULL,
				RemisionSN = 1,
				EnPlanta = (CASE WHEN @pnClaUbicacion = 55 THEN (CASE WHEN EsNacional = 1 THEN 1 ELSE 0 END) ELSE 1 END),
				CopiaTranspSN = 0,
				ClaViaje = idViaje, --@nIdViaje, 
				ClaFabricacion = IdFabricacion,
				IdFactura
			FROM #tmpFabricacionFactura 

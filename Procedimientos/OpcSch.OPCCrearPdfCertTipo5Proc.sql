USE Operacion
GO
-- EXEC SP_HELPTEXT 'OpcSch.OPCCrearPdfCertTipo5Proc'
GO
ALTER PROCEDURE OpcSch.OPCCrearPdfCertTipo5Proc
@pnNumVersion		INT,
@pnClaUbicacion		INT,
@pnIdCertificado	INT,
...
BEGIN
	...
	IF ISNULL(@psRutaProcedimiento, '') = ''
		SELECT @psRutaProcedimiento = 'OpcSch.OPCCrearPdfCertTipo5Proc'
    ELSE
		SELECT @psRutaProcedimiento = LTRIM(RTRIM(@psRutaProcedimiento)) + ' - ' + 'OpcSch.OPCCrearPdfCertTipo5Proc'
   ...
   SELECT @sNomArticulo = replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(@sNomArticulo, 
	...
	SET @sComandoDinamico = 'echo 	Dim parameters(19) As ParameterValue												>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output
	...
	SET @sComandoDinamico = 'echo 	parameters(17).Value = "2"															>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output

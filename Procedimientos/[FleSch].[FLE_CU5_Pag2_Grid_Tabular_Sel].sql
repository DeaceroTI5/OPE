...
----       
 ...
 OR      (@pnEsNinguno = 0 AND (@pnEsPorViaje = 1 AND (@pnNumViaje IS NULL OR t0.Referencia1 = CONVERT(VARCHAR(200), @pnNumViaje) )) )             
 OR      (@pnEsNinguno = 0 AND (@pnEsPorBoleta = 1 AND (t0.Referencia1 LIKE '%'+LTRIM(RTRIM(ISNULL(@psNumBoleta,'')))+'%')) )            

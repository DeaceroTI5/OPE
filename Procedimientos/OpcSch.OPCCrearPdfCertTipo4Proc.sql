USE Operacion
GO
-- EXEC SP_HELPTEXT 'OpcSch.OPCCrearPdfCertTipo4Proc'
GO
ALTER PROCEDURE OpcSch.OPCCrearPdfCertTipo4Proc
@pnNumVersion		INT,
@pnClaUbicacion		INT,
@pnIdCertificado	INT,
...
BEGIN
	...
	IF @pnDebug = 1
		SELECT 'PROCEDIMIENTO OpcSch.OPCCrearPdfCertTipo4Proc'
	...
	IF ISNULL(@psRutaProcedimiento, '') = ''
		SELECT @psRutaProcedimiento = 'OpcSch.OPCCrearPdfCertTipo4Proc'
    ELSE
		SELECT @psRutaProcedimiento = LTRIM(RTRIM(@psRutaProcedimiento)) + ' - ' + 'OpcSch.OPCCrearPdfCertTipo4Proc'
	...
	DECLARE	@nClaCliente		INT,
			...
			@sConstruccion		VARCHAR(100),
			@sGrado				VARCHAR(100),
			@sLubrication		VARCHAR(100),
			@sCoreType			VARCHAR(100),
			@sTorcido			VARCHAR(100),
			@sAcabado			VARCHAR(100),
			@sTipoConstrucion	VARCHAR(100)
						...
						CASE	WHEN ISNULL(cfgFam.ClaFamilia,0) > 0 -- Si se encuentra dentro de la config. de Familias OPC
								...
								ELSE	-- No se encuentra dentro de la config. de Familias OPC
									CASE	WHEN ciudadpedido.ClaPais = 1 
					 				...
					 				END
					END,
--			@sNombreUbicacion = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LTRIM(RTRIM(UPPER(ISNULL(ubi.NombreUbicacion, ' ')))),'Á','A'),'É','E'),'Í','I'),'Ó','O'),'Ú','U'),
			...
			@sNomArticulo = LTRIM(RTRIM(ISNULL(art.ClaveArticulo, ''))) + ' - ' + LTRIM(RTRIM(ISNULL(art.NomArticulo, ' '))),
			...
			@sConstruccion = ISNULL(CarCons.NomCaracteristica,''),
			@sGrado = ISNULL(CarGrado.NomCaracteristica,''),
			@sLubrication = ISNULL(CarLubricacion.NomCaracteristica,''),
			@sCoreType = ISNULL(CarTipoAlma.NomCaracteristica,''),
			@sTorcido = ISNULL(CarTorcido.NomCaracteristica,''),
			@sAcabado = ISNULL(CarAcabado.NomCaracteristica,''),
			@sTipoConstrucion = ISNULL(CarTipoArmadura.NomCaracteristica,'')
	...
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarCons			WITH(NOLOCK) ON 
			CarCons.ClaCaracteristica = ptccp.Construccion 
			And CarCons.ClaTipoInventario = 1  
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarGrado			WITH(NOLOCK) ON 
			CarGrado.ClaCaracteristica = ptccp.Grado 
			And CarGrado.ClaTipoInventario = 1
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarTipoAlma		WITH(NOLOCK) ON 
			CarTipoAlma.ClaCaracteristica = ptccp.TipoAlma 
			And CarTipoAlma.ClaTipoInventario = 1
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarTorcido		WITH(NOLOCK) ON 
			CarTorcido.ClaCaracteristica = ptccp.Torcido 
			And CarTorcido.ClaTipoInventario = 1						
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarAcabado		WITH(NOLOCK) ON 
			CarAcabado.ClaCaracteristica = ptccp.Acabado 
			And CarAcabado.ClaTipoInventario = 1  
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarTipoArmadura	WITH(NOLOCK) ON 
			CarTipoArmadura.ClaCaracteristica = ptccp.TipoArmadura 
			And CarTipoArmadura.ClaTipoInventario = 1
	LEFT	JOIN OPCSch.OPCArtCatCaracteristicaVw CarLubricacion	WITH(NOLOCK) ON 
			CarLubricacion.ClaCaracteristica = ptccp.TipoArmadura 
			And CarLubricacion.ClaTipoInventario = 1			
	...
	/*
	IF @pnDebug = 1
		SELECT '' AS '#SalidaComando', * FROM #SalidaComando
	*/
	...
	SET @sComandoDinamico = 'echo 	Dim parameters(30) As ParameterValue												>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output
	...
	SET @sComandoDinamico = 'echo 	parameters(24).Value = "' + @sGrado + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output		
	...
	SET @sComandoDinamico = 'echo 	parameters(25).Value = "' + @sConstruccion + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output			
	...
	SET @sComandoDinamico = 'echo 	parameters(26).Value = "' + @sLubrication + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output
	...
	SET @sComandoDinamico = 'echo 	parameters(27).Value = "' + @sCoreType + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output	
	...
	SET @sComandoDinamico = 'echo 	parameters(28).Value = "' + @sTorcido + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output
	...
	SET @sComandoDinamico = 'echo 	parameters(29).Value = "' + @sAcabado + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output	
	...
	SET @sComandoDinamico = 'echo 	parameters(30).Value = "' + @sTipoConstrucion + '"								>> "'+@sNomArchivoRSS+'"'
	EXEC master.dbo.xp_cmdshell @sComandoDinamico, no_output	
	...
	IF @pnDebug = 1
		SELECT @sComandoDinamico AS '@sComandoDinamico pdf'
	...
	IF @pnDebug = 1 
		SELECT @nExisteArchivo as '@nExisteArchivo'

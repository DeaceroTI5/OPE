...
BEGIN      
    ...
    WHERE   br.IdFabricacion = @pnIdFabVentaDet
    GROUP BY
            er.ClaCliente,
    ...
    WHERE   bdr.IdFabricacionDet = @pnIdFabDetVentaDet
    AND     art.ClaArticulo = @pnArticuloDet
    GROUP BY
            a.ClaCliente,
            a.Cliente,
            a.ClaProyecto,
    ...
    WHERE   ir.IdViaje = @pnViajeDet
    AND     fr.IdFacturaAlfanumerico = @psRemisionDet
    ------Fin: Proceso PreEjecucion de Consulta------

    ------Inicio: Proceso de Consulta Encabezado------
    SELECT	fac.FechaRegistro AS ColFechaTra,
            trar.IdFabricacion AS ColFabricacionTra,
            a.Articulo AS ColClaArticuloTra,
            trar.FolioProforma AS ColFacturaTra,
            ISNULL( trar.CantSurtida, 0.00) * a.Peso_Teorico_Kgs AS ColKilosFacturadosTra,
            ROUND(ISNULL(( a.Precio_Lista * ROUND(trar.CantSurtida, 0) ), 0.00), 2) AS ColImporteTra, 
            cu.NombreUsuario + ' ' + cu.ApellidoPaterno AS ColRealizadoPor
			,a.Precio_Lista AS ColPrecioLista
    FROM	#TempVistaEstimacionesDetalleRemision a
        LEFT JOIN	OpeSch.OpeTraRelFacturaRemisionEstimacionDetVw trar WITH(NOLOCK)
            ON trar.IdViaje = a.Viaje_VO AND trar.RemisionAlfanumerico = a.Remision AND trar.ClaArticulo = a.Articulo
        LEFT JOIN	OpeSch.OpeTraFacturaEstimacionDetVw facd WITH(NOLOCK)
            ON facd.IdEstimacionFactura = trar.IdEstimacionFactura AND facd.IdFabricacion = trar.IdFabricacion AND facd.IdFabricacionDet = trar.IdFabricacionDet AND facd.ClaArticulo = trar.ClaArticulo
        LEFT JOIN	OpeSch.OpeTraFacturaEstimacionVw fac WITH(NOLOCK)
            ON fac.IdEstimacionFactura = facd.IdEstimacionFactura AND fac.IdFabricacion = trar.IdFabricacion AND fac.ClaCliente = a.ClaCliente AND fac.ClaProyecto = a.ClaProyecto
        LEFT JOIN	OpeSch.TiCatUsuarioVw cu WITH(NOLOCK)
            ON cu.ClaUsuario = trar.ClaUsuarioMod  
    ORDER BY
        trar.FolioProforma

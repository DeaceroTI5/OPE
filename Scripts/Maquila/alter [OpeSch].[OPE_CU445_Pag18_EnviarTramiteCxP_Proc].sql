...
set @pnEsDebug=1
IF @pnEsDebug=1
select 'Procedimiento: ' as '[OpeSch].[OPE_CU445_Pag18_EnviarTramiteCxP_Proc]', @pnIdTramiteMaquilador as '@pnIdTramiteMaquilador'
 ...
 IF(@pnEsDebug = 1)
 BEGIN
	SELECT @nClaSistemaOrigen AS '@nClaSistemaOrigen'
 END
...
E.IdRecepOrdenMaquilaDet = A.IdRecepOrdenMaquilaDet AND  
 ...
 IF(@pnEsDebug = 1)
 BEGIN
	SELECT '' AS '@CXPTmpEntradaMaquila', * FROM @CXPTmpEntradaMaquila
END
 ...
 GROUP BY   
   ...
   IF @pnEsDebug = 1
	SELECT '' AS '@CXPTmpEntradaMaquilaDet', * FROM @CXPTmpEntradaMaquilaDet
 ...
 WHERE ISNULL(t1.EnviadoCXP, 0) = 0 AND t1.ClaEstatus IN (2,3)--SE LIBERO O SE CERRO PERO NO SE ENVIO A CXP  
   ...
   IF @pnEsDebug = 1 
	SELECT '' AS '@Contratos', * FROM @Contratos
 ...
 BEGIN  
  ...
  IF @pnEsDebug = 1
	select @psMensajeError as '@psMensajeError 340'
 ...
 BEGIN  
  ...
  PRINT @psMensajeError 
	...
	select '' as 'Debug- CxP_CU1_Pag15_RecibirEntradaMaquilaEnc_Srv', @IdControlRecepcion as '@IdControlRecepcion',@IdControlRecepcionUnico as '@IdControlRecepcionUnico' , @psMensajeError as '@psMensajeError', @@ERROR as '@@ERROR'
 END 
...
--  PRINT 'SELECT * FROM @CXPTmpEntradaMaquila'
  SELECT ''AS'@CXPTmpEntradaMaquila update', * FROM @CXPTmpEntradaMaquila
--  PRINT 'SELECT * FROM @CXPTmpEntradaMaquilaDet'  
  SELECT ''AS'@CXPTmpEntradaMaquilaDet update',* FROM @CXPTmpEntradaMaquilaDet  
 END  
 --------------------------------------------------------------  
 ...
 IF @pnEsDebug = 1  
 BEGIN  
  SELECT ''as'CXP_6OFGRALES_LNKSVR.CxP.CxpSch.CXPTmpEntradaMaquila',* FROM CXP_6OFGRALES_LNKSVR.CxP.CxpSch.CXPTmpEntradaMaquila   
 ...
 END  
    ...
    IF @pnEsDebug = 1
	select @psMensajeError as '@psMensajeError 510'
 ...
 END  
   ...
   IF @pnEsDebug = 1
	select @psMensajeError as '@psMensajeError 613'
 ...
 BEGIN TRY  
  ...
  BEGIN 
  IF @pnEsDebug=1
	SELECT '' as 'Debug CXP_6OFGRALES_LNKSVR.CxP.CxpSch.[CxP_CU1_Pag15_RecibirEntradaMaquilaAut_Srv]', @pnError as '@pnError', @psMensajeError as '@psMensajeError', @nMovsAutorizados as '@nMovsAutorizados', @nMovsConError as '@nMovsConError'
 ...
 PRINT ERROR_MESSAGE()  
  ...
  GOTO FIN 
    ...
    IF @pnEsDebug = 1
	select @psMensajeError as '@psMensajeError 780'
